...
class Clip(tfds.core.GeneratorBasedBuilder):
    ...
    def _transform_example(self, image_file):
        ...
        interpolation = cv2.INTER_AREA if np.mean(image.shape[:2]) > self.image_size else cv2.INTER_CUBIC
        image = cv2.resize(image, (self.image_size, self.image_size), interpolation=interpolation)
