...
class Refiner:
    def __init__(self, hub_uri):
        ...
        self.coord = tf.Variable(
            shape=(1, None, None, 2), dtype='float32', initial_value=np.zeros((1, 0, 0, 2)).astype('float32'))
    ...
    def _safe_predict(self, image, mask, coord=None):
        ...
        if coord is not None and (coord.min() < -1. or coord.max() > 1.):
        ...
        if not (coord is None or 0 == h_pad or 0 == w_pad):
            raise ValueError('Inputs shape should be divisible by 32 if coord provided')
    ...
    def make_coord(batch, height, width, dtype='float32'):
        height_ = 1. / np.array(height, 'float32')
        width_ = 1. / np.array(width, 'float32')

        vertical = height_ - 1. + 2 * height_ * np.arange(height, dtype='float32')
        horizontal = width_ - 1. + 2 * width_ * np.arange(width, dtype='float32')
