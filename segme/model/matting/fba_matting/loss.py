...
from segme.loss import MeanAbsoluteRegressionError, GradientMeanSquaredError, LaplacianPyramidLoss, \
    ReflectionTransmissionExclusionLoss, WeightedLossFunctionWrapper


def _mae(y_true, y_pred, sample_weight):
    return MeanAbsoluteRegressionError()(y_true, y_pred, sample_weight=sample_weight)


def _lap(y_true, y_pred, sample_weight):
    return LaplacianPyramidLoss(sigma=1.056, weight_pooling='max')(y_true, y_pred, sample_weight=sample_weight)
