...
from ...common import HeadActivation, HeadProjection, SameConv, ResizeByScale, resize_by_scale
...
class TriTransNet(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.transition8 = SameConv(64, 3, activation='relu')
        self.transition16 = SameConv(64, 3, activation='relu')
        self.transition32 = SameConv(64, 3, activation='relu')

        self.ufm16_up = SameConv(64, 3, activation='relu')
        self.ufm16_proj = SameConv(64, 3, activation='relu')
        self.ufm32_up0 = SameConv(64, 3, activation='relu')
        self.ufm32_proj0 = SameConv(64, 3, activation='relu')
        self.ufm32_up1 = SameConv(64, 3, activation='relu')
        self.ufm32_proj1 = SameConv(64, 3, activation='relu')

        self.vit = VisionTransformer()

        self.deconv8 = models.Sequential([SameConv(64, 3, activation='relu'), ResizeByScale(2)])
        self.deconv16 = models.Sequential([SameConv(64, 3, activation='relu'), ResizeByScale(2)])
        self.deconv32 = models.Sequential([SameConv(64, 3, activation='relu'), ResizeByScale(2)])
        ...
        self.deconv48 = models.Sequential([SameConv(128, 3, activation='relu'), ResizeByScale(2)])
        self.deconv416 = models.Sequential([SameConv(128, 3, activation='relu'), ResizeByScale(2)])
        self.deconv432 = models.Sequential([SameConv(128, 3, activation='relu'), ResizeByScale(2)])
        self.deconv248 = models.Sequential([SameConv(64, 3, activation='relu'), ResizeByScale(2)])
        self.deconv2416 = models.Sequential([SameConv(64, 3, activation='relu'), ResizeByScale(2)])
        self.deconv2432 = models.Sequential([SameConv(64, 3, activation='relu'), ResizeByScale(2)])

        self.predict8 = models.Sequential([
            SameConv(32, 3, activation='relu'),
            # ResizeByScale(2),
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        feats8_vr = tf.concat([feats8_vit, feats8_ts], axis=-1)
        feats8_vrd = self.deconv8(feats8_vr)

        feats16_vr = tf.concat([feats16_vit, feats16_proj], axis=-1)
        feats16_vrd = self.deconv16(feats16_vr)

        feats32_vr = tf.concat([feats32_vit, feats32_proj1], axis=-1)
        feats32_vrd = self.deconv32(feats32_vr)

        feats48_comb = tf.concat([feats8_vrd, feats4], axis=-1)
        feats48_combd = self.deconv48(feats48_comb)

        feats248_comb = tf.concat([feats48_combd, feats2], axis=-1)
        feats248_combd = self.deconv248(feats248_comb)
        logits8 = self.predict8(feats248_combd)
        preds8 = self.act(logits8)

        feats416_comb = tf.concat([feats16_vrd, feats4], axis=-1)
        feats416_comb3 = self.deconv416(feats416_comb)

        feats2416_comb = tf.concat([feats416_comb3, feats2], axis=-1)
        feats2416_combd = self.deconv2416(feats2416_comb)
        logits16 = self.predict16(feats2416_combd)
        preds16 = self.act(logits16)

        feats432_comb = tf.concat([feats32_vrd, feats4], axis=-1)
        feats432_combd = self.deconv432(feats432_comb)

        feats2432_comb = tf.concat([feats432_combd, feats2], axis=-1)
