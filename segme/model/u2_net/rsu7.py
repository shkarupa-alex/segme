...
class RSU7(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.cbr6 = ConvNormAct(self.mid_features, 3)
        self.cbr7 = ConvNormAct(self.mid_features, 3, dilation_rate=2)

        self.cbr6du = SmoothInterpolation(None)
        self.cbr5du = SmoothInterpolation(None)
        self.cbr4du = SmoothInterpolation(None)
        self.cbr3du = SmoothInterpolation(None)
        self.cbr2du = SmoothInterpolation(None)

        self.cbr6d = ConvNormAct(self.mid_features, 3)
    ...
    def call(self, inputs, **kwargs):
        ...
        outputs6dup = self.cbr6du([outputs6d, outputs5])

        outputs5d = self.cbr5d(tf.concat([outputs6dup, outputs5], axis=-1))
        outputs5dup = self.cbr5du([outputs5d, outputs4])

        outputs4d = self.cbr4d(tf.concat([outputs5dup, outputs4], axis=-1))
        outputs4dup = self.cbr4du([outputs4d, outputs3])
