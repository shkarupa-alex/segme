...
def Encoder():
    ...
    ext_config = patch_config(
        base_config, [0], "batch_input_shape", lambda old: old[:-1] + (4,)
    )
    ext_config = patch_config(
        ext_config, [2], "mean", lambda old: old + [0.449]
    )
    ext_config = patch_config(
        ext_config, [2], "variance", lambda old: old + [0.497**2]
    )
...
def ExpRef(sup_unfold=False):
    ...
    model = models.Model(
        inputs=inputs, outputs=[probs2, probs4, probs8], name="exp_sod"
    )
