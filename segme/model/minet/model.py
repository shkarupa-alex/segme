...
from ...common import ClassificationHead, ResizeByScale, resize_by_sample
...
class MINet(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        out5 = self.upconv32(layers.add([self.sim32(out5), out5]))  # 1024

        out4 = layers.add([resize_by_sample([out5, out4]), out4], align_corners=False)  # 1024
        out4 = self.upconv16(layers.add([self.sim16(out4), out4]))

        out3 = layers.add([resize_by_sample([out4, out3]), out3], align_corners=False)
        out3 = self.upconv8(layers.add([self.sim8(out3), out3]))  # 512

        out2 = layers.add([resize_by_sample([out3, out2]), out2], align_corners=False)
        out2 = self.upconv4(layers.add([self.sim4(out2), out2]))  # 256

        out1 = layers.add([resize_by_sample([out2, out1]), out1], align_corners=False)
        out1 = self.upconv2(layers.add([self.sim2(out1), out1]))  # 64

        outputs = self.upconv1(self.up2(out1))  # 32
...
def build_minet(channels, classes, bone_arch='resnet_50', bone_init='imagenet', bone_train=False):
