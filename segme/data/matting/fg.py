...
def _solve_fg_level(level, alpha, afg, fg, height, width, levels, kappa, steps):
    ...
    denom = ops.square(alpha_) + da0 + da1 + da2 + da3
    term0 = alpha_ * afg_ / denom

    if isinstance(level, float):
        steps_ = int(2 ** min(levels - level, 5) * steps)
    else:
        steps_ = ops.cast(2 ** ops.minimum(levels - level, 5) * steps, "int32")

    if isinstance(steps_, int):
        for _ in range(steps_):
            fg, _, _, _, _, _, _ = _solve_fg_step(
...
def solve_fg(image, alpha, kappa=1.0, steps=16, name=None):
    with backend.name_scope(name or "solve_fg"):
        ...
        if isinstance(height, int) and isinstance(width, int):
            levels = np.ceil(ops.log(max(height, width)) / np.log(2)).item()
        else:
        ...
        if isinstance(levels, float):
            for level in range(int(levels)):
                _, _, _, fg, _, _, _, _, _ = _solve_fg_level(
                    level + 1.0,
                    alpha,
                    afg,
                    fg,
