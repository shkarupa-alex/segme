...
class TestSequence(testing.TestCase):
    def test_layer(self):
        self.run_layer_test(
            Sequence,
            init_kwargs={"items": [layers.BatchNormalization(), layers.ReLU()]},
    ...
    def test_mask_produce(self):
        inputs = np.array([[0, 1, 2], [2, 1, 0]]).astype("int32")
        ...
        self.assertTrue(
            (mask == np.array([[False, True, True], [True, True, False]])).all()
        )
    ...
    def test_mask_pass(self):
        ...
        outputs = Sequence(
            [layers.Dense(6), layers.ReLU(), Split2(), layers.Minimum()]
        )(inputs, mask=masks)
        self.assertTrue(hasattr(outputs, "_keras_mask"))
