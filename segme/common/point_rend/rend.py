...
class PointRend(layers.Layer):
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        image_shape, _ = get_shape(inputs[0], axis=[1, 2])
        coarse_shape, _ = get_shape(inputs[1], axis=[1, 2])
        assertions = [
            tf.debugging.assert_less_equal(
                coarse_shape[0] * 2, image_shape[0],
    ...
    def _eval(self, inputs):
        ...
        image_shape, _ = get_shape(input_images, axis=[0, 1, 2], dtype='float32')
        coarse_shape, _ = get_shape(coarse_features, axis=[1, 2], dtype='float32')
        subdivisions = tf.cast(tf.round(tf.maximum(
            image_shape[1] / coarse_shape[0], image_shape[2] / coarse_shape[1])), 'int32')
        ...
        _, predict_logits, point_logits, point_coords = tf.while_loop(
            ...
            [
                ...
                tf.zeros((image_shape[0], 0, self.classes), self.compute_dtype),  # point logits
                tf.zeros((image_shape[0], 0, 2), self.compute_dtype)  # point coords
            ],
            shape_invariants=[
                tf.TensorShape([]),
    ...
    def _subdiv(self, iteration, coarse_features, fine_features, prev_logits, prev_coords):
        ...
        (logits_batch, logits_height, logits_width), _ = get_shape(coarse_features, axis=[0, 1, 2])
        flat_logits = tf.reshape(coarse_features, [logits_batch, logits_height * logits_width, -1])

        (batch_size, point_size), _ = get_shape(point_indices, axis=[0, 1])
