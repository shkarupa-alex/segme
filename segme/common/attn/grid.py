...
class GridAttention(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        outputs = with_divisible_pad(
            lambda padded, pad_size, pad_val: self.qkv_part(padded, pad_size, pad_val),
    ...
    def qkv_part(self, qkv, pad_size, pad_val):
        ...
        if self.qk_units == self.v_units:
            qkv = partition_apply_fused(qkv, pad_height, pad_width, 'grid_size', self.current_window, self.num_heads)
            ...
            q = partition_apply_fused(
                q, pad_height, pad_width, 'grid_size', self.current_window, self.num_heads, qkv_mult=1)
            k = partition_apply_fused(
                k, pad_height, pad_width, 'grid_size', self.current_window, self.num_heads, qkv_mult=1)
            v = partition_apply_fused(
                v, pad_height, pad_width, 'grid_size', self.current_window, self.num_heads, qkv_mult=1)

        outputs = self.qkv_attn(q, k, v, pad_size=pad_size, pad_val=pad_val)
        outputs = partition_reverse_fused(
            outputs, pad_height, pad_width, 'grid_size', self.current_window, self.num_heads)
