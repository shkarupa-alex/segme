...
class AdaptiveAveragePooling(layers.Layer):
    ...
    def case_static(self, inputs, height, width):
        start_h = np.arange(self.output_size[0], dtype='float32') * height / self.output_size[0]
        start_h = start_h.astype('int32')
        stop_h = (np.arange(self.output_size[0], dtype='float32') + 1) * height / self.output_size[0]
        stop_h = np.ceil(stop_h).astype('int32')
        size_h = stop_h - start_h
        over_h = stop_h[:-1] - start_h[1:]

        start_w = np.arange(self.output_size[1], dtype='float32') * width / self.output_size[1]
        start_w = start_w.astype('int32')
        stop_w = (np.arange(self.output_size[1], dtype=np.float32) + 1) * width / self.output_size[1]
    ...
    def case_dynamic_nondivisible(self, inputs, height, width):
        ...
        stop_h = (tf.range(self.output_size[0], dtype='float32') + 1) * tf.cast(height, 'float32') / self.output_size[0]
        ...
        start_w = tf.range(self.output_size[1], dtype='float32') * tf.cast(width, 'float32') / self.output_size[1]
        start_w = tf.cast(start_w, 'int32')
        stop_w = (tf.range(self.output_size[1], dtype='float32') + 1) * tf.cast(width, 'float32') / self.output_size[1]
