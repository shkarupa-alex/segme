...
def grid_sample(
    features, grid, mode="bilinear", align_corners=False, symmetric_pad=False
):
    if mode not in {"bilinear", "nearest"}:
        raise ValueError(
            'Wrong interpolation mode. Only "bilinear" and "nearest" supported'
        )
    ...
    with tf.control_dependencies(assertions):
        ...
        safe_features = tf.pad(
            features, [[0, 0], [1, 1], [1, 1], [0, 0]], mode=pad_mode
        )
        ...
        if "bilinear" == mode:
            ...
            ne = tf.math.reduce_prod(
                (grid_sw - grid) * [1, -1], axis=-1, keepdims=True
            )
            sw = tf.math.reduce_prod(
...
def make_coords(inputs, dtype=None):
    dtype = dtype or dtype_policy.dtype_policy().compute_dtype
    ...
    height_ = 1.0 / tf.cast(height, dtype)
    width_ = 1.0 / tf.cast(width, dtype)

    vertical = (
        height_
        - 1.0
        + 2 * height_ * tf.cast(tf.range(height, dtype="float32"), dtype)
    )
    horizontal = (
        width_
        - 1.0
        + 2 * width_ * tf.cast(tf.range(width, dtype="float32"), dtype)
    )
...
def query_features(
    ...
    feat_coords = make_coords(features, features.dtype)
    ...
    if local_ensemble:
        outputs = [
            tf.cast(pred, "float32") * area for pred, area in zip(preds, areas)
        ]
