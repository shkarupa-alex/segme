...
class CalibratedFocalSigmoidCrossEntropy(LossFunctionWrapper):
    ...
    def __init__(
            ...
            reduction=tf.keras.losses.Reduction.AUTO, name='calibrated_focal_sigmoid_cross_entropy'):
        super().__init__(
...
def calibrated_focal_sigmoid_cross_entropy(y_true, y_pred, prob0, prob1, gamma0, gamma1, alpha, from_logits):
    ...
    prob1 = tf.convert_to_tensor(prob1, dtype=tf.keras.backend.floatx())
    ...
    gamma3 = tf.convert_to_tensor(0.0, dtype=tf.keras.backend.floatx())
