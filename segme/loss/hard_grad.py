...
class HardGradientMeanAbsoluteError(WeightedLossFunctionWrapper):
    ...
    def __init__(self, reduction=Reduction.AUTO, name='hard_gradient_mean_absolute_error'):
        super().__init__(hard_gradient_mean_absolute_error, reduction=reduction, name=name)
...
def hard_gradient_mean_absolute_error(y_true, y_pred, sample_weight):
    ...
    loss = weighted_loss(tf.abs(g_true_x - g_pred_x), g_weight_x) + \
