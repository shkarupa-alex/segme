...
def preserve_current_layer_policies(cls):
    if not issubclass(cls, layers.Layer):
        raise ValueError(
            f"Can preserve policies only in `Layer` subclass. Got {cls}"
        )
    ...
    setattr(
        cls, "get_config", _get_config_decorator(getattr(cls, "get_config"))
    )
...
def _init_decorator(fn):
    ...
    def decorate(self, *args, **kwargs):
        self.__alpol = alpol.deserialize(
            kwargs.pop("__alpol", alpol.global_policy())
        )
        self.__bbpol = bbpol.deserialize(
            kwargs.pop("__bbpol", bbpol.global_policy())
        )
        self.__cnapol = cnapol.deserialize(
