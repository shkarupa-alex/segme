...
from tf_keras.src.applications.efficientnet_v2 import CONV_KERNEL_INITIALIZER
...
def Stem(filters, depth, path_drop=0., path_gamma=1., name=None):
    if name is None:
...
def Reduce(filters, fused=False, kernel_size=3, expand_ratio=4., name=None):
    if name is None:
...
def LocalBlock(
        filters, window_size, num_heads, qk_units=16, dilation_rate=1, kernel_size=3, path_drop=0., expand_ratio=3.,
...
def CoMA(
        stem_dim, stem_depth, embed_dim, stage_depths, pretrain_window, current_window=None, path_gamma=0.01,
        path_drop=0., pretrain_size=384, current_size=None, input_shape=None, include_top=True, model_name='coma',
    ...
    x = Stem(stem_dim, stem_depth, path_drop=stem_drops, path_gamma=stem_gammas, name='stem')(x)
    ...
    for i, stage_depth in enumerate(stage_depths):
        ...
        for j in range(stage_depth):
            ...
            if 0 == j % 3:
                x = SwinBlock(
                    stage_dim, stage_window, pretrain_window, num_heads, 0, path_drop=stage_drops[j],
                    expand_ratio=expand_ratio, path_gamma=stage_gammas[j], name=f'stage_{i}_attn_{j}')(x)
            elif 1 == j % 3:
                shift_mode = (sum((d + 1) // 3 for d in stage_depths[:i]) + j // 3) % 4 + 1
                x = SwinBlock(
                    stage_dim, stage_window, pretrain_window, num_heads, shift_mode, path_drop=stage_drops[j],
                    expand_ratio=expand_ratio, path_gamma=stage_gammas[j], name=f'stage_{i}_attn_{j}')(x)
            else:
                x = LocalBlock(
                    stage_dim, 5, num_heads, dilation_rate=1, kernel_size=3, path_drop=stage_drops[j],
                    expand_ratio=expand_ratio, path_gamma=stage_gammas[j], name=f'stage_{i}_attn_{j}')(x)
...
def CoMATiny(
        stem_dim=32, stem_depth=2, embed_dim=64, stage_depths=(3, 6, 21, 3), path_drop=0.1, pretrain_window=16,
        pretrain_size=256, model_name='coma_tiny', weights=None, classes=14607, classifier_activation='linear',
        **kwargs):
    # 26.1 13.7
    with cnapol.policy_scope('conv-ln-gelu'):
        return CoMA(
            stem_dim=stem_dim, stem_depth=stem_depth, embed_dim=embed_dim, stage_depths=stage_depths,
            path_drop=path_drop, pretrain_window=pretrain_window, pretrain_size=pretrain_size, model_name=model_name,
            weights=weights, classes=classes, classifier_activation=classifier_activation, **kwargs)


def CoMASmall(
        stem_dim=48, stem_depth=2, embed_dim=96, stage_depths=(3, 6, 21, 3), path_drop=0.2, pretrain_window=16,
        pretrain_size=256, model_name='coma_small', weights=None, classes=14607, classifier_activation='linear',
        **kwargs):
    # 56.3 28.0
    with cnapol.policy_scope('conv-ln-gelu'):
        return CoMA(
...
def CoMABase(
        stem_dim=64, stem_depth=3, embed_dim=128, stage_depths=(3, 6, 21, 3), path_drop=0.3, current_window=24,
        pretrain_window=12, model_name='coma_base', weights=None, classes=14607, classifier_activation='linear',
        **kwargs):
    # 98.1 53.6 @ 256
    with cnapol.policy_scope('conv-ln-gelu'):
        return CoMA(
            stem_dim=stem_dim, stem_depth=stem_depth, embed_dim=embed_dim, stage_depths=stage_depths,
            path_drop=path_drop, current_window=current_window, pretrain_window=pretrain_window, model_name=model_name,
            weights=weights, classes=classes, classifier_activation=classifier_activation, **kwargs)


def CoMALarge(
        stem_dim=80, stem_depth=4, embed_dim=160, stage_depths=(6, 9, 27, 3), path_drop=0.4, current_window=24,
        pretrain_window=12, model_name='coma_large', weights=None, classes=14607, classifier_activation='linear',
        **kwargs):
    # 172.1 108.5 @ 256
    with cnapol.policy_scope('conv-ln-gelu'):
        return CoMA(
