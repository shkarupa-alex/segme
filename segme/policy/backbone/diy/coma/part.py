...
def halo_partition(inputs, height, width, window_size, halo_size, dilation_rate=1, dtype=None, name=None):
    with tf.name_scope(name or 'halo_partition'):
        ...
        outputs = tf.reshape(outputs, [-1, (halo_size ** 2), channels])
...
def partition_apply_fused(inputs, height, width, part_type, size_count, qkv_size, num_heads, dilation_rate=1,
    ...
    with tf.name_scope(name or 'partition_apply_fused'):
        ...
        if part_type in {'window_size', 'grid_size'}:
            outputs = tf.reshape(
                outputs, [-1, num_heads, (size_count ** 2), qkv_size, head_channels])
        else:
            outputs = tf.reshape(
                outputs, [-1, num_heads, height_blocks * width_blocks, qkv_size, head_channels])
...
def halo_partition_fused(inputs, height, width, window_size, halo_size, qkv_size, num_heads, dilation_rate=1,
    ...
    with tf.name_scope(name or 'halo_partition_fused'):
        ...
        outputs = tf.reshape(outputs, [-1, num_heads, halo_size ** 2, qkv_size, head_channels])
