...
from segme.policy.conv import CONVOLUTIONS, FixedConv, FixedDepthwiseConv, SpectralConv
...
class TestStandardizedConv(test_combinations.TestCase):
    ...
    def test_value(self):
        ...
        layer = CONVOLUTIONS.new('stdconv', 1, 3)
        layer.build(inputs.shape)
        layer.set_weights([kernel, bias])

        model = models.Sequential([layer])
        model.compile(optimizers.SGD(1e-9), 'mae')
        model.fit(inputs, np.zeros((1, 1, 2, 1), 'float32'))
    ...
    def test_batch(self):
        ...
        inputs = np.random.normal(size=(32, 16, 16, 64)) * 10.
        layer = CONVOLUTIONS.new('stdconv', 64, 3)

        model = models.Sequential([layer])
        model.compile(optimizers.SGD(1e-9), 'mae')
        model.fit(inputs, np.zeros((32, 14, 14, 64), 'float32'))
        ...
        result = np.concatenate(result, axis=0)
        self.assertTrue(np.all(np.isfinite(result)))
        self.assertAllClose(expected, result, atol=9e-4)
