...
from segme.policy.sameconv import SAMECONVS, SameConv, SameStandardizedConv, SameSpectralConv, SameDepthwiseConv, \
    SameStandardizedDepthwiseConv, SameSpectralDepthwiseConv
...
class TestSamConvsRegistry(unittest.TestCase):
    def test_filled(self):
        self.assertEqual(
            str(SAMECONVS),
            'LayerRegistry ({'
            '\'conv\': <class \'segme.policy.sameconv.SameConv\'>, '
            '\'stdconv\': <class \'segme.policy.sameconv.SameStandardizedConv\'>, '
            '\'snconv\': <class \'segme.policy.sameconv.SameSpectralConv\'>, '
            '\'dwconv\': <class \'segme.policy.sameconv.SameDepthwiseConv\'>, '
            '\'dwstdconv\': <class \'segme.policy.sameconv.SameStandardizedDepthwiseConv\'>, '
            '\'dwsnconv\': <class \'segme.policy.sameconv.SameSpectralDepthwiseConv\'>})')
...
@test_combinations.run_all_keras_modes
class TestSameSpectralDepthwiseConv(test_combinations.TestCase):
    def setUp(self):
        super(TestSameSpectralDepthwiseConv, self).setUp()
        self.default_policy = mixed_precision.global_policy()

    def tearDown(self):
        super(TestSameSpectralDepthwiseConv, self).tearDown()
        mixed_precision.set_global_policy(self.default_policy)

    def test_layer(self):
        test_utils.layer_test(
            SameSpectralDepthwiseConv,
            kwargs={'kernel_size': 1, 'strides': 1},
            input_shape=[2, 16, 16, 8],
            input_dtype='float32',
            expected_output_shape=[None, 16, 16, 8],
