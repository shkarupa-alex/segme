...
class LayerRegistry(Registry):
    def _validate(self, key, value):
        ...
        if isinstance(value, dict):
            if "class_name" not in value:
                raise ValueError(
                    f"Expected serialized layer config, got {value}"
                )
    ...
    def _kwargs(self, cls):
        ...
        kwargs = {
            k
            for k, _ in signature.parameters.items()
    ...
    def new(self, key, *args, **kwargs):
        ...
        if is_class:
            ...
            value["module"] = value.get("module", "keras.layers")
            cls = api_export.get_symbol_from_name(
                f"{value['module']}.{value['class_name']}"
            ) or get_registered_object(value["class_name"])
...
class BackboneRegistry(Registry):
    ...
    def _validate(self, key, value):
        ...
        if not isinstance(value, tuple):
            raise ValueError(
                f"Only a tuples of (model function, endpoints) "
                f"can be registered, got {value}"
            )
        if not callable(value[0]):
            raise ValueError(
                f"Model function should be callable, got {value[0]}"
            )
        if 6 != len(value[1]):
    ...
    def new(self, arch, init, channels, scales):
        ...
        if scales is None:
            ...
            if None in curr_endpoints:
                bad_idxs = [
                    fi for fi, uf in enumerate(curr_endpoints) if uf is None
                ]
