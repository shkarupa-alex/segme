...
| Classification Checkpoint| MACs(M)| Parameters(M)| Top1 Accuracy| Pixel1(ms)|
...
@generic_utils.register_keras_serializable(package='SegMe')
def mobilenet_v3_hard_sigmoid(x):
    return layers.ReLU(6.)(x + 3.) * (1. / 6.)


@generic_utils.register_keras_serializable(package='SegMe')
def mobilenet_v3_hard_swish(x):
    return layers.Multiply()([
        layers.Activation('SegMe>mobilenet_v3_hard_sigmoid')(x), x])
...
def _se_block(inputs, filters, se_ratio, prefix):
    ...
    x = layers.Activation('SegMe>mobilenet_v3_hard_sigmoid')(x)
...
def MobileNetV3(stack_fn,
    ...
    Arguments:
      ...
      input_shape: optional shape tuple, to be specified if you would like to
        use a model with an input img resolution that is not (224, 224, 3). It
        should have exactly 3 inputs channels (224, 224, 3). You can also omit
        this option if you would like to infer input_shape from an input_tensor.
        If you choose to include both input_tensor and input_shape then
        input_shape will be used if they match, if the shapes do not match then
        we will throw an error. E.g. `(160, 160, 3)` would be one valid value.
      alpha: controls the width of the network. This is known as the depth
        ...
        - If `alpha` > 1.0, proportionally increases the number of filters in
          each layer.
        - If `alpha` = 1, default number of filters from the paper are used at
          each layer.
    ...
    Raises:
      ValueError: in case of invalid model type, argument for `weights`, or
        invalid input shape when weights='imagenet'
    """
    if 'layers' in kwargs:
        global layers
    ...
    if weights == 'imagenet':
        if minimalistic is False and alpha not in [0.75, 1.0] \
            ...
            raise ValueError('If imagenet weights are being loaded, alpha'
                             ' can be one of `0.75`, `1.0` for non minimalistic'
    ...
    if minimalistic:
        ...
        kernel = 5
        activation = 'SegMe>mobilenet_v3_hard_swish'
        se_ratio = 0.25
    ...
    # Create model.
    model = training.Model(inputs, x, name='MobilenetV3' + model_type)
...
MobileNetV3Large.__doc__ = MobileNetV3Large.__doc__
