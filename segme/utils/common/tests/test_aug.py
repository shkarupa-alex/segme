...
class TestAugmentOnTheFly(tf.test.TestCase):
    ...
    def test_no_aug(self):
        images, masks = augment_onthefly(self.images, self.masks, hflip_prob=0., vflip_prob=0., brightness_prob=0.,
                                         contrast_prob=0., hue_prob=0., saturation_prob=0.)
    ...
    def test_aug(self):
        kwargs = {'hflip_prob': 0., 'vflip_prob': 0., 'brightness_prob': 0., 'contrast_prob': 0., 'hue_prob': 0.,
                  'saturation_prob': 0.}
        ...
        for aug in kwargs.keys():
            ...
            if aug in {'hflip_prob', 'vflip_prob'}:
                for i in range(len(self.masks)):
                    mask = self.evaluate(masks[i])
