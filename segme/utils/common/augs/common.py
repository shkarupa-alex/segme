...
def apply(image, masks, weight, prob, image_fn, mask_fn, weight_fn, name=None):
    ...
    with tf.name_scope(name or "apply"):
        ...
        square_size = (
            height == width if static_size else tf.equal(height, width)
        )
...
def transform(
    ...
    with tf.name_scope(name or "transform"):
        ...
        output_shape = tf.convert_to_tensor(
            output_shape, "int32", name="output_shape"
        )
        fill_value = tf.convert_to_tensor(
            fill_value, "float32", name="fill_value"
        )
    ...
    with tf.name_scope(name or "validate"):
        ...
        if masks is not None:
            ...
            for i, mask in enumerate(masks):
                mask_ = tf.convert_to_tensor(
                    mask, dtype=mask.dtype, name=f"mask_{i}"
                )
