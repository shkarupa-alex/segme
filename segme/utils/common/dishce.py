...
def _filter_boundaries(contours, mask, condition):
    ...
    for i in range(len(contours)):
        ...
        for j in range(0, contours[i].shape[0]):
            ...
            independent[components[r, c]] = 0  # Part of the boundary of this region needs human correction
...
def _approximate_rdp(boundaries, epsilon=1.):
...
def _relax_hce(y_true, y_pred, y_true_skeleton, relax=5, epsilon=2.0):
    ...
    union_ = cv2.erode(union.astype('uint8'), disk(1), iterations=relax).astype('bool')
    ...
    contours_fp, _ = cv2.findContours(fp_.astype('uint8'), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    boundaries_fp, indep_fp_cnt = _filter_boundaries(contours_fp, fp_, tp | fn_)

    contours_fn, _ = cv2.findContours(fn_.astype('uint8'), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    boundaries_fn, indep_fn_cnt = _filter_boundaries(contours_fn, fn_, ~(tp | fp_ | fn_))
