...
def cut_mix_up(image, label, weight, classes, cutmix_alpha=1., mixup_alpha=0.3, cutmix_prob=0.5, mixup_prob=0.5,
               name=None):
    with tf.name_scope(name or 'cut_mix_up'):
        ...
        apply = tf.random.uniform([batch], 0., 1.)
        cutmix_apply = apply < cutmix_prob
        mixup_apply = apply > 1. - cutmix_prob

        image_, label_, weight_ = cut_mix(image, label, weight, cutmix_alpha, classes)
        image = tf.where(cutmix_apply[..., None, None, None], image_, image)
        label = tf.where(tf.reshape(cutmix_apply, [batch] + [1] * (label.shape.rank - 1)), label_, label)
        if weight is not None:
            weight = tf.where(tf.reshape(cutmix_apply, [batch] + [1] * (label.shape.rank - 1)), weight_, weight)

        image_, label_, weight_ = mix_up(image, label, weight, cutmix_alpha, classes)
        image = tf.where(mixup_apply[..., None, None, None], image_, image)
        label = tf.where(tf.reshape(mixup_apply, [batch] + [1] * (label.shape.rank - 1)), label_, label)
        if weight is not None:
            weight = tf.where(tf.reshape(mixup_apply, [batch] + [1] * (label.shape.rank - 1)), weight_, weight)
    ...
    with tf.name_scope(name or 'cut_mix'):
        ...
        if weight is not None:
            ...
            if weight.shape.rank != label.shape.rank:
                raise ValueError('Expecting `weight` and `label` ranks to be equal.')
        ...
        cutsize = tf.round(fullsize * tf.math.sqrt(1. - factor[..., None]))
        topleft = tf.round(tf.random.uniform(shape=(batch, 2), maxval=fullsize - cutsize))

        return _cut_mix(image, label, weight, order, topleft, cutsize, classes, name=None)
...
def _cut_mix(image, label, weight, order, topleft, cutsize, classes, name=None):
    with tf.name_scope(name or 'cut_mix_'):
        ...
        bottom, right = tf.split(tf.cast(cutsize + topleft, 'int32'), 2, axis=-1)
        factor = tf.reduce_prod(cutsize, axis=-1, keepdims=True) / tf.cast(height * width, 'float32')
        ...
        label += tf.reshape(factor, [-1] + [1] * (label.shape.rank - 1)) * (label_ - label)

        if weight is not None:
            weight_ = tf.gather(weight, order)
            weight = weight + tf.reshape(factor, [-1] + [1] * (weight.shape.rank - 1)) * (weight_ - weight)
...
def _mix_up(image, label, weight, order, factor, classes, name=None):
    with tf.name_scope(name or 'mix_up_'):
        ...
        image_ = tf.gather(image, order)
        image += tf.reshape(factor, [-1] + [1] * (image.shape.rank - 1)) * (image_ - image)
        ...
        if weight is not None:
            ...
            weight_ = tf.gather(weight, order)
            weight += tf.reshape(factor, [-1] + [1] * (weight.shape.rank - 1)) * (weight_ - weight)
