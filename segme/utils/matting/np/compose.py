...
def compose_two(fg0, alpha0, fg1, alpha1, crop=False, solve=True):
    ...
    interpolation = (
        cv2.INTER_AREA
        if min(alpha1.shape[:2]) > min(alpha0.shape[:2])
        else cv2.INTER_LANCZOS4
    )

    # Resize background to be compatible with foreground
    alpha1 = cv2.resize(
        alpha1, alpha0.shape[1::-1], interpolation=interpolation
    )

    # Switch to float after resizing to beat overflow
    alpha0, alpha1 = (
        alpha0.astype("float32") / 255.0,
        alpha1.astype("float32") / 255.0,
    )
    ...
    if (alpha == 0.0).all() or (alpha == 1.0).all():
