...
def compose_two(fg, alpha, solve=True, name=None):
    with tf.name_scope(name or "compose_two"):
        ...
        accept = tf.reduce_any(alpha_ > 0.0, axis=[1, 2, 3]) & tf.reduce_any(
        ...
        fg_ = tf.clip_by_value(fg_, 0.0, 1.0)

        fg_ = tf.cast(tf.round(fg_ * 255.0), "uint8")
        alpha_ = tf.cast(tf.round(alpha_ * 255.0), "uint8")
