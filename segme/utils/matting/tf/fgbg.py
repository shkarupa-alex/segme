...
def _solve_fgbg_step(step, image, fg, bg, alpha, grad_weight, regularization):
    ...
    inv_det = 1. / (a00 * a11 - a01 ** 2)
    b00 = a11 * inv_det
    b01 = -a01 * inv_det
    b11 = a00 * inv_det

    fg = tf.clip_by_value(b00 * b0 + b01 * b1, 0., 1.)
    bg = tf.clip_by_value(b01 * b0 + b11 * b1, 0., 1.)
...
def solve_fgbg(image, alpha, regularization=0.005, small_size=32, small_steps=10, big_steps=2, grad_weight=0.1,
    ...
    with tf.name_scope(name or 'solve_fgbg'):
        ...
        fg = tf.cast(tf.round(fg * 255.), 'uint8')
        bg = tf.cast(tf.round(bg * 255.), 'uint8')
