...
def solve_fgbg(image, alpha, regularization=0.005, small_size=32, small_steps=10, big_steps=2, grad_weight=0.1):
    ...
    for level in range(levels + 1):
        ...
        image_ = cv2.resize(image, (width_, height_), interpolation=cv2.INTER_LINEAR)
        alpha_ = cv2.resize(alpha, (width_, height_), interpolation=cv2.INTER_LINEAR)
        ...
        steps = small_steps if width_ <= small_size and height_ <= small_size else big_steps
        ...
        for step in range(steps):
            ...
            b0 += da0 * pad_fg[1:-1, :-2] + da1 * pad_fg[1:-1, 2:] + \
                  da2 * pad_fg[:-2, 1:-1] + da3 * pad_fg[2:, 1:-1]
            b1 += da0 * pad_bg[1:-1, :-2] + da1 * pad_bg[1:-1, 2:] + \
                  da2 * pad_bg[:-2, 1:-1] + da3 * pad_bg[2:, 1:-1]
            ...
            fg = np.clip(b00 * b0 + b01 * b1, 0., 1.)
            bg = np.clip(b01 * b0 + b11 * b1, 0., 1.)

    fg = np.round(fg * 255.).astype('uint8')
    bg = np.round(bg * 255.).astype('uint8')
